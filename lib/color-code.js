(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ColorCode = {}));
    ...
    ***************************************************************************** */
    ...
    var GamepadInteractor = (function (_super) {
        __extends$1(GamepadInteractor, _super);
    ...
    var CloneAudioManager = (function (_super) {
        __extends$1(CloneAudioManager, _super);
    ...
    var AudioManager = (function (_super) {
        __extends$1(AudioManager, _super);
    ...
    var EntityManager = (function (_super) {
        __extends$1(EntityManager, _super);
    ...
    var SceneManager = (function (_super) {
        __extends$1(SceneManager, _super);
    ...
    function debugCenter$1(context, entity) {
        var body = entity.getBodyBox();
        var x = entity.x + entity.scene.camera.x;
        var y = entity.y + entity.scene.camera.y;
        context.beginPath();
        context.fillStyle = "#f00";
        context.arc(x, y, 2, 0, Math.PI * 2);
        context.fill();
        context.closePath();
        context.translate((entity.width * entity.getScale().x) / -2, (entity.height * entity.getScale().y) / -2);
        context.translate((entity.width / 2) * -entity.getOrigin().x * entity.getScale().x, (entity.height / 2) * -entity.getOrigin().y * entity.getScale().y);
        context.strokeStyle = "green";
        context.strokeRect(x + body.getX(), y + body.getY(), body.getWidth(), body.getHeight());
        context.setTransform(1, 0, 0, 1, 0, 0);
    }
    ...
    var ColorCodeImageEntity = (function (_super) {
        __extends(ColorCodeImageEntity, _super);
        function ColorCodeImageEntity() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this._keepAlpha = true;
            _this._update = true;
            _this._imageAlpha = 1;
            _this._changeColorMap = new Map();
            _this._prevUse = "";
            return _this;
        }
        ColorCodeImageEntity.prototype.keepAlpha = function (value) {
            this._keepAlpha = value;
            return this;
        };
        ColorCodeImageEntity.prototype.setAlpha = function (value) {
            this._imageAlpha = value;
            return this;
        };
        ColorCodeImageEntity.prototype.getAlpha = function () {
            return this._imageAlpha;
        };
        ColorCodeImageEntity.prototype.changeColor = function (from, to) {
            this._changeColorMap.set(from, to);
            this._update = true;
            return this;
        };
        ColorCodeImageEntity.prototype.removeColor = function (from) {
            this._changeColorMap = new Map(Array.from(this._changeColorMap.entries()).filter(function (v) { return v[0][0] !== from[0] && v[0][1] !== from[1] && v[0][2] !== from[2]; }));
            this._update = true;
            return this;
        };
        ColorCodeImageEntity.prototype.draw = function (context) {
            var _this = this;
            var image = this.manager.medias.images.get(this.use);
            context.globalAlpha =
                this.alpha * (this.scene.isPlayed === "opacity" ? this.scene.alpha : 1);
            if (!this.fixed)
                context.translate(this.scene.camera.x, this.scene.camera.y);
            context.translate((this.width * this.getScale().x) / -2, (this.height * this.getScale().y) / -2);
            context.translate((this.width / 2) * -this.originX * this.getScale().x, (this.height / 2) * -this.originY * this.getScale().y);
            if (isDefined$1(image)) {
                if (!isDefined$1(this.width) && !isDefined$1(this.height)) {
                    this.width = image.width;
                    this.height = image.height;
                }
                if (this._prevUse !== this.use || this._update) {
                    this.getImageData(image).then(function (image) {
                        _this._prevImage = image;
                    });
                    this._prevUse = this.use;
                    this._update = false;
                }
                context.drawImage(this._prevImage || image, 0, 0, this.width * this.getCrop().w, this.height * this.getCrop().h, this.x - this.scene.camera.x, this.y - this.scene.camera.y, this.width * this.getScale().x, this.height * this.getScale().y);
            }
            context.setTransform(1, 0, 0, 1, 0, 0);
            if (this.scene.game.debug)
                debugCenter$1(context, this);
        };
        ColorCodeImageEntity.prototype.getImageData = function (image) {
            var _this = this;
            return new Promise(function (resolve) {
                var canvas = document.createElement("canvas");
                canvas.width = _this.width;
                canvas.height = _this.height;
                canvas.style.imageRendering = isChromium$1() ? "pixelated" : "crisp-edges";
                var context = canvas.getContext("2d");
                context.drawImage(image, 0, 0, _this.width, _this.height);
                var imageData = context.getImageData(0, 0, canvas.width, canvas.height);
                var changeColor = Array.from(_this._changeColorMap.keys());
                var _loop_1 = function (i) {
                    var toColor = _this._changeColorMap.get(changeColor.find(function (rgb) {
                        return rgb[0] === imageData.data[i] &&
                    ...
                    if (toColor) {
                        ...
                        if (!_this._keepAlpha)
                            imageData.data[i + 3] = _this._imageAlpha;
                    }
                };
        ...
        return ColorCodeImageEntity;
    }(index$1.ObjectEntities.Image));

    var ColorCodeSpriteEntity = (function (_super) {
        __extends(ColorCodeSpriteEntity, _super);
        function ColorCodeSpriteEntity() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this._keepAlpha = true;
            _this._update = true;
            _this._imageAlpha = 1;
        ...
        ColorCodeSpriteEntity.prototype.keepAlpha = function (value) {
            this._keepAlpha = value;
            return this;
        };
        ColorCodeSpriteEntity.prototype.setAlpha = function (value) {
            this._imageAlpha = value;
            return this;
        };
        ColorCodeSpriteEntity.prototype.getAlpha = function () {
            return this._imageAlpha;
        };
        ColorCodeSpriteEntity.prototype.changeColor = function (from, to) {
            this._changeColorMap.set(from, to);
            this._update = true;
            return this;
        };
        ColorCodeSpriteEntity.prototype.removeColor = function (from) {
            this._changeColorMap = new Map(Array.from(this._changeColorMap.entries()).filter(function (v) { return v[0][0] !== from[0] && v[0][1] !== from[1] && v[0][2] !== from[2]; }));
            this._update = true;
            return this;
        };
        ColorCodeSpriteEntity.prototype.draw = function (context) {
            var _this = this;
            var image = this.manager.medias.images.get(this.use);
            context.globalAlpha =
                this.alpha * (this.scene.isPlayed === "opacity" ? this.scene.alpha : 1);
            if (!this.fixed)
                context.translate(this.scene.camera.x, this.scene.camera.y);
            context.translate((this.width * this.getScale().x) / -2, (this.height * this.getScale().y) / -2);
            context.translate(this.width * this.originX * this.getScale().x, this.height * this.originY * this.getScale().y);
            if (isDefined$1(image)) {
                if (!isDefined$1(this.width) && !isDefined$1(this.height)) {
                    this.width = image.width;
                    this.height = image.height;
        ...
        ColorCodeSpriteEntity.prototype.getImageData = function (image) {
            var _this = this;
            return new Promise(function (resolve) {
                var canvas = document.createElement("canvas");
                canvas.width = _this.width;
                ...
                canvas.style.imageRendering = isChromium$1() ? "pixelated" : "crisp-edges";
                var context = canvas.getContext("2d");
                context.drawImage(image, _this.sprite.x, _this.sprite.y, _this.sprite.width, _this.sprite.height, 0, 0, _this.width, _this.height);
                var imageData = context.getImageData(0, 0, canvas.width, canvas.height);
                var changeColor = Array.from(_this._changeColorMap.keys());
                var _loop_1 = function (i) {
                    var toColor = _this._changeColorMap.get(changeColor.find(function (rgb) {
        ...
        return ColorCodeSpriteEntity;
    }(index$1.ObjectEntities.Sprite));

    exports.ColorCodeImageEntity = ColorCodeImageEntity;
    exports.ColorCodeSpriteEntity = ColorCodeSpriteEntity;
